...
1. **Wallet leg** — The member must have a **registered wallet** (`/set-wallet`). The bot uses the [MultiversX public API](https://api.multiversx.com) to count NFTs in the configured collections on that address. If the wallet API call fails for that run, the wallet leg **fails** (the role is not granted on ambiguous data).
