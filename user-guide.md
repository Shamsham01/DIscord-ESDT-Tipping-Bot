...
If your server uses **NFT holder roles** (admins configure this with **`/nft-role-verification`**), eligibility can depend on linked-wallet MvX holdings, Virtual Account NFT inventory, or both added together (**`wallet_or_va`** is the **`create`** default)—see **[NFT Role Verification](nft-role-verification.md)**.
